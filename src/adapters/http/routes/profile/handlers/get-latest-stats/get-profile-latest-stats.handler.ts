...
import {
  getProfileLatestStatusQueryDTOSchema,
  type GetProfileLatestStatusQueryDTO
...
export class GetProfileLatestStatusHandler implements GetProfileLatestStatusHandlerPort {
  ...
  async handle(
    ...
    const parsed = await getProfileLatestStatusQueryDTOSchema.safeParseAsync(ctx.request.query);
    if (!parsed.success) {
